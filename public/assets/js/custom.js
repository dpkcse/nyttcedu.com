$(function() {
	...
	! function(n, e, i, a) {
		n.navigation = function(t, s) {
			...
			n(t).find(".nav-menus-wrapper").prepend(""), n(t).find(".nav-search").length > 0 && n(t).find(".nav-search").find("form").prepend("<span class='nav-search-close-button'>✕</span>"), u.init = function() {
...
$(document).on('click', '#subsBtn', function() {
	...
	if(subscribe_email == '') {		
		$('#subscribeErr').text("Email address field can't be empty.");
		$('#subscribeSuc').text("");
	} else {
		if(isEmail(subscribe_email)) {
			...
			$.ajax({
				...
				success: function(res, status, xhr) {
					if(res.status) {						
						$('#subscribeSuc').text(res.sucMsg);
					} else {
						$('#subscribeErr').text(res.errMsg);
					}
				},
			...
			$('#subscribeErr').text("Invalid email address.");
			$('#subscribeSuc').text("");
		}
	}
});
